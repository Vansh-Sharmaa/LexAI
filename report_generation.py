...
from utils import get_resend_credentials
...
def send_email(recipient_email, attachment=None, subject=None, body=None, attachment_name=None):
    ...
    Uses Resend's built-in test sender (onboarding@resend.dev) which works
    on the free plan without a verified custom domain.
    Returns a tuple of (success_boolean, message_string)
    """
    try:
        resend_api_key, _ = get_resend_credentials()
        resend.api_key = resend_api_key
    except ValueError as e:
        return False, f"⚠ Missing API credentials: {e}"

    # Use Resend's own verified test sender — avoids the domain-verification error
    # on the free plan. Change to a custom verified domain in production.
    SENDER = "LexAI <onboarding@resend.dev>"

    if subject is None:
        subject = "Your Legal Document Analysis Report — LexAI"
    ...
    if attachment:
        try:
            ...
            return False, f"Failed to encode attachment: {attach_err}"

    try:
        response = resend.Emails.send(params)
        email_id = response.get("id") if isinstance(response, dict) else getattr(response, "id", "N/A")
